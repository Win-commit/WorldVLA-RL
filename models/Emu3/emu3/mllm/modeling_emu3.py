...
import sys
# sys.path.append("/liujinxin/zhy/UniVLA")
...
#     """
#     Emu3 attention module using torch.nn.attention.flex_attention API.
#     This module inherits from `Emu3Attention` and overrides the forward method to use Flex Attention.
#     """
...
#         if output_attentions:
#             logger.warning_once(
#                 "Emu3Model is using Emu3FlexAttention, but returning attention weights is not currently supported. "
...
#         # 使用flex_attention正确的API调用
#         if isinstance(attention_mask, BlockMask):
#             # 如果传入的是已经创建好的BlockMask，直接使用
#             attn_output = flex_attention(
#                 query_states, 
#                 key_states, 
#                 value_states,
#                 block_mask=attention_mask,  # 注意: 这里使用block_mask参数
#                 scale=scale_factor
#             )
#         else:
#             if attention_mask is not None:
#                 # 转换为批次可用的格式
#                 batch_masks = attention_mask.squeeze(1) # [B, S, S]
                
#                 # def custom_mask_mod(b, h, q_idx, kv_idx):
#                 #     mask_val = batch_masks[b][q_idx][kv_idx]  
#                 #     return  (mask_val > -1e8)
#                 def custom_score_mod(score, b, h, q_idx, kv_idx):
#                     mask_val = batch_masks[b][q_idx][kv_idx]
#                     return torch.where(mask_val > -1e8, score, torch.tensor(-float('inf'), device=score.device, dtype=score.dtype))
...
#             # mask = create_block_mask(custom_mask_mod, bsz, None, q_len, q_len)  
#             attn_output = flex_attention(
#                 query_states,
#                 key_states,
#                 value_states,
#                 score_mod=custom_score_mod,
#                 scale=scale_factor
#             )

#         # Flex Attention的输出也是[B, H, S, D]格式，不需要转置回来
#         # 只需reshape成预期的输出形状
...
EMU3_ATTENTION_CLASSES = {
    ...
    # "flex_attention": Emu3FlexAttention,  # Add the Flex Attention implementation
}
